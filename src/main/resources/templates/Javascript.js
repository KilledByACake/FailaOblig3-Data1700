...
//Koble til java
// Funksjon for å hente alle billetter fra backend
function hentBilletter() {
    fetch('/hentAlle')
        .then(response => {
            if (!response.ok) {
                throw new Error('Kunne ikke hente billetter');
            }
            return response.json();
        })
        .then(data => {
            // Behandle mottatte billettdatabruk
            console.log('Mottatte billetter:', data);
            // Oppdater billettlistevisning om nødvendig
        })
        .catch(error => {
            console.error('Feil ved henting av billetter:', error);
            // Håndter feil om nødvendig
        });
}

// Funksjon for å slette alle billetter fra backend
function slettAlleBilletter() {
    fetch('/slettAlle', {
        ...
        .then(response => {
            if (!response.ok) {
                throw new Error('Kunne ikke slette billetter');
            }
            // Oppdater billettlistevisning eller utfør nødvendig handling
        })
        .catch(error => {
